...
def evaluate_cards(*cards: int | str | Card) -> int:
    ...
    if not (MIN_CARDS <= hand_size <= MAX_CARDS) or (hand_size
                                                     not in NO_FLUSHES):
        msg = (
...
def get_hand_category(score):
    """
    Convert a hand score into a descriptive category.
    Lower scores are better hands (Royal Flush = 1).
    """
    if score == 1:
        return "Royal Flush"
    elif score <= 10:  # Straight Flush
